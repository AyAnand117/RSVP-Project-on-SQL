...
-- The information_schema.tables gives us the ability to check all the information about the tables in a database.
...
-- It will count the null values in each column and sum them up. Highest number of null values were in worlwide_gross_income_nulls .
...
-- Type your code below:
-- First Half
SELECT  year, COUNT(id) as num_of_movie
FROM movie
GROUP BY year
ORDER BY year;

/* The highest number of movies were produced in 2017 followed by 2018 and least number of movies was produced in 2019. */
...
/* USA and India produced more than a thousand movies(1007!) in the year 2019.
...
WITH cnt_genre AS
(
	SELECT movie_id, COUNT(genre) AS num_of_genre
	FROM genre
	GROUP BY movie_id
...
/* Using with clause we first find the movies with exaclty 1 genre and then we count them all using the unique id */
...
SELECT genre, AVG(duration) as avg_dur
FROM genre AS g
INNER JOIN movie AS m
ON g.movie_id = m.id
GROUP BY genre;
...
LIMIT 3),
top_director AS(SELECT n.name AS director_name,
